        ...
        bottomNavigationView.setOnNavigationItemSelectedListener(new BottomNavigationView.OnNavigationItemSelectedListener() {
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                ...
                if (itemId == R.id.nav_input) {
                    myData.add(new MyItem("Input Clicked", R.drawable.login_background));
                    requestBody = "{\"action\": \"nav_input\", \"message\": \"User clicked on Input\"}";
                } else if (itemId == R.id.nav_output) {
                    myData.add(new MyItem("Output Clicked", R.drawable.login_background));
                    requestBody = "{\"action\": \"nav_output\", \"message\": \"User clicked on Output\"}";
                } else if (itemId == R.id.nav_suggestion) {
                    myData.add(new MyItem("Suggestion Clicked", R.drawable.login_background));
                    requestBody = "{\"action\": \"nav_suggestion\", \"message\": \"User clicked on Suggestion\"}";
                }

                if (!requestBody.isEmpty()) {
                    communicationApi(requestBody); // 调用 API 通信
                }
                return true;
            }
        });
